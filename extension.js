...
function activate(context) {
    ...
    if (process.platform == "win32") {
        ...
        if (config().get("force_sway") === false) {
            ...
            const allWindowIdsOutput = cp
                .execSync(`xprop -root | grep '_NET_CLIENT_LIST(WINDOW)'`)
                .toString();

            const allWindowIds = allWindowIdsOutput.match(/0x[\da-f]+/gi);
        ...
        function setAlpha(alpha) {
            ...
            if (config().get("force_sway") === true) {
                ...
                cp.exec(
                    `swaymsg opacity ${(alpha / 255).toFixed(2)}`,
                ...
                for (const codeWindowId of codeWindowIds) {
                    cp.exec(
                        `xprop  -id ${codeWindowId} -f _NET_WM_WINDOW_OPACITY 32c -set _NET_WM_WINDOW_OPACITY $(printf 0x%x $((0xffffffff * ${alpha} / 255)))`,
                        function (error, stdout, stderr) {
                            if (error) {
                                console.error(`GlassIt error: ${error}`);
                                return;
